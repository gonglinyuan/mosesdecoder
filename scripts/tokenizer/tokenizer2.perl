...
sub tokenize
{
    ...
    if (($language eq "fi") or ($language eq "sv")) {
        ...
        $text =~ s/([^\p{IsAlnum}\p{InCyrillic}\p{InGujarati}\s\.\'’\`\,\-])/ $1 /g;
    }

    # aggressive hyphen splitting
    if ($AGGRESSIVE)
    {
        $text =~ s/([\p{IsAlnum}\p{InCyrillic}\p{InGujarati}])\-(?=[\p{IsAlnum}\p{InCyrillic}\p{InGujarati}])/$1 \@-\@ /g;
    }
    ...
    if ($language eq "en")
    {
        ...
        $text =~ s/([^\p{IsAlpha}\p{InCyrillic}\p{InGujarati}])[\']([^\p{IsAlpha}\p{InCyrillic}\p{InGujarati}])/$1 ' $2/g;
        $text =~ s/([^\p{IsAlpha}\p{InCyrillic}\p{InGujarati}])[’]([^\p{IsAlpha}\p{InCyrillic}\p{InGujarati}])/$1 ’ $2/g;
        $text =~ s/([^\p{IsAlpha}\p{InCyrillic}\p{InGujarati}\p{IsN}])[\']([\p{IsAlpha}\p{InCyrillic}\p{InGujarati}])/$1 ' $2/g;
        $text =~ s/([^\p{IsAlpha}\p{InCyrillic}\p{InGujarati}\p{IsN}])[’]([\p{IsAlpha}\p{InCyrillic}\p{InGujarati}])/$1 ’ $2/g;
        $text =~ s/([\p{IsAlpha}\p{InCyrillic}\p{InGujarati}])[\']([^\p{IsAlpha}\p{InCyrillic}\p{InGujarati}])/$1 ' $2/g;
        $text =~ s/([\p{IsAlpha}\p{InCyrillic}\p{InGujarati}])[’]([^\p{IsAlpha}\p{InCyrillic}\p{InGujarati}])/$1 ’ $2/g;
        $text =~ s/([\p{IsAlpha}\p{InCyrillic}\p{InGujarati}])[\']([\p{IsAlpha}\p{InCyrillic}\p{InGujarati}])/$1 '$2/g;
    ...
    for (my $i=0;$i<(scalar(@words));$i++)
    {
        ...
        if ( $word =~ /^(\S+)\.$/)
        {
            ...
            elsif (($pre =~ /\./ && $pre =~ /\p{IsAlpha}\p{InCyrillic}\p{InGujarati}/) || ($NONBREAKING_PREFIX{$pre} && $NONBREAKING_PREFIX{$pre}==1) || ($i<scalar(@words)-1 && ($words[$i+1] =~ /^[\p{IsLower}]/)))
            {
                #no change
            }
...
$text =~ s=([;:@#\$%&\p{IsSc}\p{IsSo}])= $1 =g;
    ...
    $text =~ s/([\p{IsAlnum}\p{InCyrillic}\p{InGujarati}])\/([\p{IsAlnum}\p{InCyrillic}\p{InGujarati}])/$1 \@\/\@ $2/g;
    ...
    for (my $i=0;$i<(scalar(@words));$i++)
    {
        ...
        {
            my $pre = $1;
            if (($pre =~ /\./ && $pre =~ /\p{IsAlpha}\p{InCyrillic}\p{InGujarati}/) || ($NONBREAKING_PREFIX{$pre} && $NONBREAKING_PREFIX{$pre}==1) || ($i<scalar(@words)-1 && ($words[$i+1] =~ /^[\p{IsLower}]/)))
            {
                #no change
            }
            elsif (($NONBREAKING_PREFIX{$pre} && $NONBREAKING_PREFIX{$pre}==2) && ($i<scalar(@words)-1 && ($words[$i+1] =~ /^[0-9]+/)))
