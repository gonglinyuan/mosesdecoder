  ...
  $text = &deescape($text);
	...
	for ($i=0;$i<(scalar(@words));$i++) {
		if (&startsWithCJKChar($words[$i])) {
			...
			$text=$text.$words[$i];
			$prependSpace = " ";
		} elsif (($language eq "en") && ($i>0) && ($words[$i] =~ /^[\'’][\p{IsAlpha}\p{InCyrillic}\p{InGujarati}]/) && ($words[$i-1] =~ /[\p{IsAlnum}\p{InCyrillic}\p{InGujarati}]$/)) {
			...
			$text=$text.$words[$i];
			$prependSpace = " ";
		}  elsif ((($language eq "fr") ||($language eq "it")) && ($i<=(scalar(@words)-2)) && ($words[$i] =~ /[\p{IsAlpha}\p{InCyrillic}\p{InGujarati}][\'’]$/) && ($words[$i+1] =~ /^[\p{IsAlpha}\p{InCyrillic}\p{InGujarati}]/)) {
			#right-shift the contraction for French and Italian
			$text = $text.$prependSpace.$words[$i];
			$prependSpace = "";
		} elsif (($language eq "cs") && ($i<(scalar(@words)-3))
				&& ($words[$i] =~ /[\p{IsAlpha}\p{InCyrillic}\p{InGujarati}]$/)
				&& ($words[$i+1] =~ /^[-–]$/)
				&& ($words[$i+2] =~ /^li$|^mail.*/i)
				) {
			...
			if (($quoteCount{$normalized_quo} % 2) eq 0) {
				if(($language eq "en") && (($words[$i] eq "'") || ($words[$i] eq "’")) && ($i > 0) && ($words[$i-1] =~ /[s]$/)) {
...
sub detokenize_penn {
  ...
  for ($i=0;$i<(scalar(@words));$i++) {
    if ($words[$i] =~ /^[\p{IsSc}\(\[\{\¿\¡]+$/) {
      ...
      $text=$text.$words[$i];
      $prependSpace = " ";
    } elsif (($i>0) && ($words[$i] =~ /^[\'’][\p{IsAlpha}\p{InCyrillic}\p{InGujarati}]/) && ($words[$i-1] =~ /[\p{IsAlnum}\p{InCyrillic}\p{InGujarati}]$/)) {
